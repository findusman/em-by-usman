...
const migrations = [hashKeys, metaSettings]
...
export const migrate = state => {
  ...
  return !schemaVersion || schemaVersion >= SCHEMA_LATEST
    ? Promise.resolve(state)
    : // schema version not found
      !migrationIndex[schemaVersion]
      ? Promise.reject(new Error('migrate: Unrecognized schemaVersion: ' + schemaVersion))
      : // migrate schema
        (console.info(`Migrating schemaVersion ${schemaVersion}...`), migrationIndex[schemaVersion](state)).then(
          newState => {
            if (state.schemaVersion >= newState.schemaVersion) {
              throw new Error(
                'Migration Validation Error: Expected schemaVersionFrom < schemaVersionTo for migration of schemaVersion ' +
                  state.schemaVersion,
              )
            } else {
